import g from"./supercomponent.js";import{render as f,html as o}from"./lit-html.js";import u from"./env.js";import l from"./modals.js";import s from"./diagram-controller.js";import w from"./save-file.js";import"./input.js";import k from"./open-file.js";import r from"./alerts.js";import{subscribe as b}from"./pubsub.js";import c from"./settings-modal.js";import M from"./help-modal.js";class m extends g{constructor(){super();this.toggle=e=>{this.set({open:!this.model.open})};this.onReset=e=>{l.dangerous({title:"Reset the canvas?",message:"This will remove all of your work and cannot be undone.",confirm:"Reset",cancel:"Cancel",callbacks:{confirm:()=>{s.reset()}}})};this.onSave=e=>{this.save()};this.onShare=e=>{const[t,i]=s.exportAsURI(),n=`${location.origin}#data:${i}`;navigator.clipboard.writeText(n).then(()=>{r.toast("Copied to clipboard")}).catch(a=>{console.error("Clipboard failed:",a)})};this.onOpen=e=>{if(window?.showOpenFilePicker)k({description:"Schema Sketcher Diagram",mimeTypes:["application/json"],extensions:[".json"]}).then(t=>{const i=new FileReader;i.onload=n=>{const a=n.target.result;s.import(a),r.toast("File opened")},i.readAsText(t)});else{const t=document.createElement("input");t.type="file",t.accept=".json",t.onchange=i=>{const n=t.files[0],a=new FileReader;a.onload=d=>{const h=d.target.result;s.import(h),r.toast("File opened")},a.readAsText(n)},t.click()}};this.onSettings=e=>{l.raw({view:o`${new c}`,closeable:!0})};this.onHelp=e=>{l.raw({view:o`${new M}`,width:769,closeable:!0})};this.onInstall=e=>{window?.deferredInstallPrompt?(window.deferredInstallPrompt.prompt(),window.deferredInstallPrompt.userChoice.then(()=>{window.deferredInstallPrompt=null})):r.snackbar("Your browser does not support installation of web applications.")};this.file=null,this.model={open:!1,unsaved:!1},b("diagram",this.diagramInbox.bind(this))}async connected(){await u.css(["main-menu","button","modals"]).then(()=>{this.render()}),window.addEventListener("beforeunload",e=>{if(this.model.unsaved){var t="Are you sure you want to exit? You have unsaved changes.";return(e||window.event).returnValue=t,t}})}diagramInbox({type:e,data:t}){switch(e){case"dirty":this.set({unsaved:!0});break;case"load":this.file=null,this.set({unsaved:!1});break;case"reset":this.file=null,this.set({unsaved:!1});break;default:break}}async save(){const[e,t]=s.export(),i=e.fileName||"untitled";this.file!==null?(this.file=await w(new Blob([t],{type:"application/json"}),{fileName:`${e.fileName}.json`,mimeTypes:["application/json"],extensions:[".json"],id:e.uid.replace(/-/g,"")},this.file),r.toast("File saved"),this.set({unsaved:!1})):l.form({title:"Save diagram",view:o`
                    <brixi-input
                        data-value="${i}"
                        data-name="fileName"
                        data-required="true"
                    ></brixi-input>
                `,submit:"Save",callbacks:{submit:async(n,a,d)=>{if(s.setFileName(n.fileName),window?.showSaveFilePicker)this.file=await w(new Blob([t],{type:"application/json"}),{fileName:`${n.fileName}.json`,mimeTypes:["application/json"],extensions:[".json"],id:e.uid.replace(/-/g,"")},this.file);else{const h=new Blob([t],{type:"application/json"}),v=URL.createObjectURL(h),p=document.createElement("a");p.href=v,p.download=`${n.fileName}.json`,p.click(),URL.revokeObjectURL(v)}d.remove(),r.toast("File saved"),this.set({unsaved:!1})}}})}renderMenuButton(){return this.model.open?o`
        ...
        `;f(e,this),s.type==="custom"&&s.getTypes().length===0&&l.raw({view:o`${new c}`,closeable:!0})}}u.bind("main-menu",m);export{m as default};
